...
from cropgym import _DEFAULT_RESULTSDIR, _SCENARIO_PATH
...
def _make_runner_for_agent(agent: str, env: MultiFieldEnv, dict_fields: dict, render: bool = False):
    """Create the paper baseline runner for the provided baseline name."""
    return make_baseline_runner(agent, env=env, render=render)
...
def get_model_name(file_path: str) -> str:
    ...
    candidates = ["ROT", "RANDOM"]
    for c in candidates:
        if c in p:
            return "random" if c == "RANDOM" else c
...
if __name__ == "__main__":
    ...
    parser.add_argument("--agent", type=str, default=None, help="deprecated alias for --baseline")
