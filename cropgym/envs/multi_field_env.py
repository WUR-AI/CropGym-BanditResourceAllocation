...
from cropgym import _FIELDS_CONFIG, _CONFIG_PATH
...
from cropgym.utils.scenario_utils import choose_soil_type
...
class MultiFieldEnv(AECEnv, EzPickle):
    ...
    """
    Multi-field CropGym environment.
    The main idea is that each field is stepped independently while sharing farm-level
    budget and scenario state.
   ...
   # following AEC env API
    ...
    def run_until_past_season_year(
        ...
        *,
        ...
        `env_agent` must expose `get_action(agent, env=self)`.
        """
